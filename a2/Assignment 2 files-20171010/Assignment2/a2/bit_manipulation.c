...
int isCharBitSet(char c, int bitNum) {
	// compare the number and the right shifted number, then completley shift the number which will return either 1 or 0
...
void setShortBit(int bitNum, short *num) {
	// use the or bit operator to set the specified bit to one
	*num = *num | (1 << bitNum);
}
...
int countBits(short num)  {
	// set a counter to 0
	int count = 0;
	// iterate over the 16 bits(assumed since it is a short)
	for (int i=0; i<16; i++) {
		// add to the counter if the bit is "set" using our existing function
		count += isShortBitSet(num, i);
	}
	// return count
	return count;
}
...
void flipBitShort(int bitNum, short *num) {
	// use xor bit operator to set the specified bit to the opposite of what it was
	*num ^= 1 << bitNum;
}
