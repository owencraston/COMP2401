...

{
    ...
    // get the bits covered by the mask
		sum = (*num) & mask;
    // count the number of bits and if the number of bits is odd then set the parity bit P1
		if (countBits(sum) % 2 == 1) {
			setShortBit(1, num);
		}
	...
	sum = (*num) & mask;
	// count the number of bits and set the parity of P2
	if (countBits(sum) % 2 == 1) {
		setShortBit(2, num);
	}
